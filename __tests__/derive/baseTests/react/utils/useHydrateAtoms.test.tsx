  ...
  function Counter({
    ...
    return (
      <>
        ...
        <button
          onClick={() => setStatus((status) => (status === 'fulfilled' ? 'rejected' : 'fulfilled'))}
        >
          update
        </button>
      </>
...
it('useHydrateAtoms should only hydrate on first render using a Map', async () => {
  ...
  function Counter({
    ...
    useHydrateAtoms(
      new Map<typeof activeAtom | typeof countAtom, typeof initialActive | typeof initialCount>([
  ...
  rerender(
    <StrictMode>
      <Counter initialCount={65} initialActive />
    </StrictMode>,
  )
  await findByText('count: 43')
  await findByText('is active: no')
...
it('useHydrateAtoms should not trigger unnecessary re-renders', async () => {
  ...
  const { findByText, getByText } = render(<Counter initialCount={42} />)
...
it('useHydrateAtoms should respect onMount', async () => {
  ...
  const { findByText } = render(<Counter initialCount={42} />)
...
it('passing dangerouslyForceHydrate to useHydrateAtoms will re-hydrated atoms', async () => {
  ...
  rerender(
    <StrictMode>
      <Counter initialCount={11} initialStatus="rejected" dangerouslyForceHydrate />
    </StrictMode>,
  )
