...
const buildStore = (
  ...
  const setAtomStateValueOrPromise = (
    ...
    if (isPromiseLike(valueOrPromise)) {
      ...
      for (const /** a dependency (`atomState.d`) of `atom` */ a of atomState.d.keys()) {
  ...
  const readAtomState = <Value>(
    ...
    if (!force?.(atom) && isAtomStateInitialized(atomState)) {
      ...
      if (
        Array.from(atomState.d).every(
          (keyValue) => {
            /** a dependency (`atomState.d`) of `atom` */
            const a = keyValue[0]
            const n = keyValue[1]
            // Recursively, read the atom state of the dependency, and
            // check if the atom epoch number is unchanged
            return readAtomState(
              pending,
              a,
              /** original atomState here is the first call to readAtomState */
              getAtomState(a, atomState),
              force
            ).n === n
          })
    ...
    atomState.d.clear()
    let isSync = true
    const getter: Getter = <V>(/** a dependency passed to `getter` */ a: Atom<V>) => {
  ...
  const getDependents = <Value>(
    ...
    for (const a of atomState.m?.t || []) {
      /**
       * original atomState is the atomState of the atom targeted by setter,
       * or the of the atom dependent deeply on the atom targeted by setter
       * passed from `recomputeDependents`
       */
  ...
  const writeAtomState = <Value, Args extends unknown[], Result>(
    ...
    const getter: Getter = <V>(a: Atom<V>) => {
      /** original atomState is the atomState of the atom targeted by setter */
      const aState = getAtomState(a, atomState)
      return returnAtomValue(readAtomState(pending, a, aState))
    }
