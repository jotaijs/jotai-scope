...
it('works with 2 level dependencies', async () => {
  ...
  function Counter() {
    ...
    return (
      <>
        <div>
          commits: {useCommitCount()}, count: {count}, doubled: {doubledCount}, tripled:{' '}
          {tripledCount}
        </div>
  ...
  function Counter() {
    ...
    return (
      <>
        ...
        <button onClick={() => dispatch(() => setCount((c) => c + 1))}>button</button>
      </>
    )
  }
...
it('should not call read function for unmounted atoms in StrictMode (#2076)', async () => {
  const countAtom = atom(1)
  let firstDerivedFn: (((get: Getter) => number) & { mockClear: () => void }) | undefined
