...
it('no unnecessary updates when updating atoms', async () => {
  ...
  function TaskItem({ itemAtom }: { itemAtom: PrimitiveAtom<TodoItem> }) {
    const [value, onChange] = useAtom(itemAtom)
    const toggle = () => onChange((value) => ({ ...value, checked: !value.checked }))
    return (
      <li>
  ...
  const { getByTestId, getByText } = render(<TaskList listAtom={todosAtom} />)
...
it('inserting atoms', async () => {
  ...
  await waitFor(() => {
    expect(queryByTestId('list')?.textContent).toBe('get cat food+get dragon food+help nana+')
  })
...
it('moving atoms', async () => {
  ...
  await waitFor(() => {
    expect(queryByTestId('list')?.textContent).toBe('get cat food<>help nana<>get dragon food<>')
  })

  await userEvent.click(getByTestId('get cat food-rightbutton'))
  await waitFor(() => {
    expect(queryByTestId('list')?.textContent).toBe('help nana<>get cat food<>get dragon food<>')
  })

  await userEvent.click(getByTestId('get cat food-rightbutton'))
  await waitFor(() => {
    expect(queryByTestId('list')?.textContent).toBe('help nana<>get dragon food<>get cat food<>')
  })

  await userEvent.click(getByTestId('help nana-leftbutton'))
  await waitFor(() => {
    expect(queryByTestId('list')?.textContent).toBe('get dragon food<>get cat food<>help nana<>')
  })
})
...
it('read-only array atom', async () => {
  ...
  function TaskItem({ itemAtom }: { itemAtom: Atom<TodoItem> }) {
    const [value] = useAtom(itemAtom)
    return (
      <li>
        <input
...
it('no error with cached atoms (fix 510)', async () => {
  ...
  function NumItem({ atom }: NumItemProps) {
...
it('variable sized splitted atom', async () => {
  ...
  const derivativeAtom = atom((get) => get(collectionAtomsAtom).map((ca) => get(ca)))
