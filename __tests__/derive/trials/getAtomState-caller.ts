import type { AtomState, AnyAtom, AnyWritableAtom, Store } from 'src/ScopeProvider2/types'
import { atom, createStore, SetStateAction, type Getter, type Setter } from 'jotai'
...
const deriveCallback: DeriveCallack = jest.fn((baseGetAtomState: GetAtomStateExtended) => {
  getAtomState = jest.fn((atom, originalAtomState) => {
    ...
    if (
      ...
      (explicit.has(baseAtomState.caller) || implicit.has(baseAtomState.caller))
    ) {
      implicit.add(atom)
    }
  ...
  atomReadTrap = jest.fn((atom, getter, options) => {
    const atomReadGetter: any = jest.fn((a) => {
  ...
  atomWriteTrap = jest.fn((atom, getter, setter, ...params) => {
    const atomWriteGetter: any = jest.fn((a) => {
      baseGetAtomState(a).caller = atom
      try {
        return getter(a)
      } finally {
    ...
    const atomWriteSetter: any = jest.fn((a, ...v) => {
      baseGetAtomState(a).caller = atom
      try {
        return setter(a, ...v)
      } finally {
...
function nthReadParams(nthCall: number, guessParams: any[] = []) {
  return Object.assign([], atomReadTrap.mock.calls[nthCall - 1]!.slice(), guessParams)
}
function nthWriteParams(nthCall: number, guessParams: any[] = []) {
  return Object.assign([], atomWriteTrap.mock.calls[nthCall - 1]!.slice(), guessParams)
}
function getAccessor<T, V>(
  ...
  return (nthCall: number) => map.get(trap.mock.calls[nthCall - 1]![paramIndex])!
}
...
describe('calls GAS and accessor traps on', () => {
  ...
  it('determines the correct atomState for implicit atoms', () => {
    ...
    expect(atomReadTrap).toHaveBeenNthCalledWith(1, ...nthReadParams(1, [atomB]))
  ...
  it('first store.get(primitiveAtom)', () => {
    ...
    expect(atomReadTrap).toHaveBeenCalledTimes(1)
    expect(atomReadTrap).toHaveBeenNthCalledWith(1, ...nthReadParams(1, [atomA]))
    expect(nthAtomReadGetter(1)).toHaveBeenCalledTimes(1)
    expect(nthAtomReadGetter(1)).toHaveBeenCalledWith(atomA)
  })
  ...
  it('store.get(asyncDerivedAtom)', async () => {
    ...
    expect(atomReadTrap).toHaveBeenCalledTimes(1)
    expect(atomReadTrap).toHaveBeenNthCalledWith(1, ...nthReadParams(1, [atomD]))
    jest.clearAllMocks()
    resolve(1)
    await 'microtask'
  ...
  it('store.set(primitiveAtom, value)', () => {
    ...
    expect(atomWriteTrap).toHaveBeenNthCalledWith(1, ...nthWriteParams(1, [atomA, , , 1]))
  ...
  it('store.set(primitiveAtom, (currValue) => nextValue)', () => {
    ...
    expect(atomReadTrap).toHaveBeenCalledTimes(0)
    expect(atomWriteTrap).toHaveBeenCalledTimes(1)
    expect(atomWriteTrap).toHaveBeenNthCalledWith(1, ...nthWriteParams(1, [atomA, , , increment]))
  ...
  it('store.set(writableAtom, value)', () => {
    ...
    expect(atomReadTrap).toHaveBeenCalledTimes(1)
    expect(atomReadTrap).toHaveBeenNthCalledWith(1, ...nthReadParams(1, [atomC]))
    expect(nthAtomReadGetter(1)).toHaveBeenCalledTimes(1)
    expect(nthAtomReadGetter(1)).toHaveBeenCalledWith(atomC)
    expect(atomWriteTrap).toHaveBeenCalledTimes(2)
    expect(atomWriteTrap).toHaveBeenNthCalledWith(1, ...nthWriteParams(1, [atomC, , , 3]))
    expect(nthAtomWriteGetter(1)).toHaveBeenCalledTimes(0)
    expect(nthAtomWriteSetter(1)).toHaveBeenCalledTimes(1)
    expect(nthAtomWriteSetter(1)).toHaveBeenCalledWith(atomA, 3)
    expect(atomWriteTrap).toHaveBeenNthCalledWith(2, ...nthWriteParams(2, [atomA, , , 3]))
  ...
  it('store.set(writableAtom, currValue => nextValue)', () => {
    ...
    expect(atomReadTrap).toHaveBeenCalledTimes(0)
    expect(atomWriteTrap).toHaveBeenCalledTimes(2)
    expect(atomWriteTrap).toHaveBeenNthCalledWith(1, ...nthWriteParams(1, [atomC, , , increment]))
    expect(nthAtomWriteGetter(1)).toHaveBeenCalledTimes(0)
    expect(nthAtomWriteSetter(1)).toHaveBeenCalledTimes(1)
    expect(nthAtomWriteSetter(1)).toHaveBeenCalledWith(atomA, increment)
    expect(atomWriteTrap).toHaveBeenNthCalledWith(2, ...nthWriteParams(2, [atomA, , , increment]))
  ...
  it('setSelf', async () => {
    ...
    expect(atomReadTrap).toHaveBeenCalledTimes(1)
    expect(atomReadTrap).toHaveBeenNthCalledWith(1, ...nthReadParams(1, [atomE]))
  ...
  it('nested atom read and write', () => {
    ...
    expect(atomReadTrap).toHaveBeenNthCalledWith(1, ...nthReadParams(1, [atomH]))
    expect(nthAtomReadGetter(1)).toHaveBeenCalledTimes(1)
    expect(nthAtomReadGetter(1)).toHaveBeenCalledWith(atomG)
    expect(atomReadTrap).toHaveBeenNthCalledWith(2, ...nthReadParams(2, [atomG]))
    expect(nthAtomReadGetter(2)).toHaveBeenCalledTimes(1)
    expect(nthAtomReadGetter(2)).toHaveBeenCalledWith(atomF)
    expect(atomReadTrap).toHaveBeenNthCalledWith(3, ...nthReadParams(3, [atomF]))
    ...
    expect(atomWriteTrap).toHaveBeenNthCalledWith(1, ...nthWriteParams(1, [atomH, , , 0]))
    ...
    expect(atomWriteTrap).toHaveBeenNthCalledWith(2, ...nthWriteParams(2, [atomG, , , 0]))
    ...
    expect(atomWriteTrap).toHaveBeenNthCalledWith(3, ...nthWriteParams(3, [atomF, , , 0]))
