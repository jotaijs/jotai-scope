...
describe('throws an error while updating in effect cleanup', () => {
  ...
  it('[DEV-ONLY] single setCount', async () => {
    ...
    await findByText('no error')
    expect(errorMessages).not.toContain('Error: Uncaught [Error: err_in_effect_cleanup]')

    await userEvent.click(getByText('close'))
    if (reactVersion.startsWith('17.')) {
      expect(errorMessages).toContain('Error: Uncaught [Error: err_in_effect_cleanup]')
    } else {
