...
const MODE = 'development' as 'production' | 'development'
...
const returnAtomValue = <Value>(atomState: AtomState<Value>): Value => {
  ...
  if (MODE !== 'production' && !('v' in atomState)) {
...
const addPendingAtom = (pending: Pending, atom: AnyAtom, atomState: AtomState) => {
...
const addPendingDependent = (pending: Pending, atom: AnyAtom, dependent: AnyAtom) => {
...
const getPendingDependents = (pending: Pending, atom: AnyAtom) => pending[0].get(atom)
...
type GetAtomState = <Value>(atom: Atom<Value>, originAtom?: AnyAtom) => AtomState<Value>
...
const buildStore = (
  ...
  const setAtomStateValueOrPromise = (
    ...
    if (isPromiseLike(valueOrPromise)) {
      ...
      for (const a of atomState.d.keys()) {
        addPendingPromiseToDependency(atom, valueOrPromise, getAtomState(a, atom))
      }
  ...
  const readAtomState = <Value>(
      ...
      if (
        Array.from(atomState.d).every(
          ([a, n]) =>
            ...
            readAtomState(pending, a, getAtomState(a, atom), force).n === n,
        )
      ) {
        return atomState
      }
    ...
    const getter: Getter = <V>(a: Atom<V>) => {
      if (isSelfAtom(atom, a)) {
        const aState = getAtomState(a, atom)
      ...
      // a !== atom
      const aState = readAtomState(pending, a, getAtomState(a, atom), force)
      if (isSync) {
    ...
    const options = {
      ...
      get setSelf() {
        if (MODE !== 'production' && !isActuallyWritableAtom(atom)) {
          console.warn('setSelf function cannot be used with read-only atom')
        }
        if (!setSelf && isActuallyWritableAtom(atom)) {
          setSelf = (...args) => {
            if (MODE !== 'production' && isSync) {
  ...
  const getDependents = <Value>(
    ...
    for (const a of atomState.m?.t || []) {
      dependents.set(a, getAtomState(a, originAtom))
    }
    for (const atomWithPendingPromise of atomState.p) {
      dependents.set(atomWithPendingPromise, getAtomState(atomWithPendingPromise, originAtom))
    }
    getPendingDependents(pending, atom)?.forEach((dependent) => {
      dependents.set(dependent, getAtomState(dependent, originAtom))
    })
    return dependents
  }
  ...
  const recomputeDependents = <Value>(
    ...
    const topsortedAtoms: (readonly [atom: AnyAtom, atomState: AtomState, epochNumber: number])[] =
      []
    ...
    const visit = (a: AnyAtom, aState: AtomState) => {
      ...
      markedAtoms.add(a)
      for (const [d, s] of getDependents(pending, a, aState, atom)) {
        if (a !== d) {
          visit(d, s)
        }
      }
    ...
    for (let i = topsortedAtoms.length - 1; i >= 0; --i) {
      ...
      if (hasChangedDeps) {
        ...
        mountDependencies(pending, a, aState, originAtom)
  ...
  const writeAtomState = <Value, Args extends unknown[], Result>(
    ...
    const getter: Getter = <V>(a: Atom<V>) =>
      returnAtomValue(readAtomState(pending, a, getAtomState(a, atom)))
    const setter: Setter = <V, As extends unknown[], R>(a: WritableAtom<V, As, R>, ...args: As) => {
      ...
      if (isSelfAtom(atom, a)) {
        ...
        mountDependencies(pending, a, aState, atom)
        if (!hasPrevValue || !Object.is(prevValue, aState.v)) {
          addPendingAtom(pending, a, aState)
          recomputeDependents(pending, a, aState, atom)
        }
      } else {
    ...
    if (atomState.m && !isPendingPromise(atomState.v)) {
      for (const a of atomState.d.keys()) {
        if (!atomState.m.d.has(a)) {
          const aMounted = mountAtom(pending, a, getAtomState(a, originAtom), atom)
          aMounted.t.add(atom)
          atomState.m.d.add(a)
        }
      }
      for (const a of atomState.m.d || []) {
        if (!atomState.d.has(a)) {
          ...
          const aMounted = unmountAtom(pending, a, getAtomState(a, originAtom), atom)
  ...
  const mountAtom = <Value>(
    ...
    if (!atomState.m) {
      ...
      readAtomState(pending, atom, atomState, undefined)
      // mount dependencies first
      for (const a of atomState.d.keys()) {
        const aMounted = mountAtom(pending, a, getAtomState(a, originAtom))
        aMounted.t.add(atom)
      }
      // mount self
      atomState.m = {
      ...
      if (isActuallyWritableAtom(atom) && atom.onMount) {
        ...
        addPendingFunction(pending, () => {
          const onUnmount = onMount((...args) => writeAtomState(pending, atom, ...args))
          if (onUnmount) {
            mounted.u = onUnmount
          }
        })
  ...
  const unmountAtom = <Value>(
    ...
    if (
      atomState.m &&
      !atomState.m.l.size &&
      !Array.from(atomState.m.t).some((a) => getAtomState(a, originAtom).m?.d.has(atom))
    ) {
      // unmount self
      ...
      for (const a of atomState.d.keys()) {
        const aMounted = unmountAtom(pending, a, getAtomState(a, originAtom))
        aMounted?.t.delete(atom)
      }
      return undefined
    }
