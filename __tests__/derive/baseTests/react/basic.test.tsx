...
it('only re-renders if value has changed', async () => {
  ...
  function Counter({ countAtom, name }: Props) {
    const [count, setCount] = useAtom(countAtom)
    return (
      <>
        <div>
...
it('uses an async write-only atom', async () => {
  const countAtom = atom(0)
  let resolve = () => {}
  const asyncCountAtom = atom(null, async (get, set, update: (prev: number) => number) => {
    await new Promise<void>((r) => (resolve = r))
...
it('can write an atom value on useEffect in children', async () => {
  ...
  function Child({ setCount }: { setCount: (f: (c: number) => number) => void }) {
    useEffect(() => {
      setCount((c) => c + 1)
    }, [setCount])
    return null
...
it('only invoke read function on use atom', async () => {
  ...
  function Counter() {
    ...
    return (
      <>
        <div>
          commits: {useCommitCount()}, count: {count}, readCount: {readCount}, doubled:{' '}
          {doubledCount}
        </div>
        <button onClick={() => setCount((c) => c + 1)}>button</button>
      </>
...
it('async chain for multiple sync and async atoms (#443)', async () => {
  ...
  const sumAtom = atom(async (get) => (await get(num1Atom)) + (await get(num2Atom)))
