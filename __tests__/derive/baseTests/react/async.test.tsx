...
it('set promise atom value on write (#304)', async () => {
  ...
  const asyncAtom = atom(null, (get, set, _arg) => {
    set(
      countAtom,
      Promise.resolve(get(countAtom)).then((c) => new Promise((r) => (resolve = () => r(c + 1)))),
    )
  })
...
it('Handles synchronously invoked async set (#375)', async () => {
  ...
  const loadDocumentAtom = atom(null, (_get, set) => {
    const fetch = async () => {
      ...
      const response = await new Promise<string>((r) => (resolve = () => r('great document')))
...
it('combine two promise atom values (#442)', async () => {
  ...
  const derivedAtom = atom(async (get) => (await get(count1Atom)) + (await get(count2Atom)))
