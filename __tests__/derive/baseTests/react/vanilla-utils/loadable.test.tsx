...
it('loadable goes back to loading after re-fetch', async () => {
  ...
  function Refresh() {
    ...
    return <button onClick={() => setRefresh((value) => value + 1)}>refresh</button>
  }
...
it('loadable of a derived async atom does not trigger infinite loop (#1114)', async () => {
  ...
  function Trigger() {
    ...
    return <button onClick={() => trigger((value) => value)}>trigger</button>
  }
...
function LoadableComponent({ asyncAtom, effectCallback }: LoadableComponentProps) {
