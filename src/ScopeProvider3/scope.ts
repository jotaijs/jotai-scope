...
import { WritableAtom } from 'jotai/ts3.8/vanilla'
...
export function createScope(
  ...
  function resolveAtom<T extends AnyAtom>(atom: AnyAtom, a: T): T {
    if (explicit.has(atom) && !explicit.has(a)) {
      const implicitAtom = emplace(a, implicitMap, Object.assign(cloneAtom, { o: a }))
...
function cloneAtom<T extends Atom<unknown>>(atom: T): T {
  return Object.create(Object.getPrototypeOf(atom), Object.getOwnPropertyDescriptors(atom))
}
...
function createAtomState<
  Value,
  T extends Partial<AtomState<Value> & Record<string, unknown>> = Record<never, never>,
>(atomState?: T) {
  const newAtomState = {
    n: 0,
    ...atomState,
