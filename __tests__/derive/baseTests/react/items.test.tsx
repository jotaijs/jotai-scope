...
it('remove an item, then add another', async () => {
  ...
  function ListItem({ itemAtom, remove }: { itemAtom: PrimitiveAtom<Item>; remove: () => void }) {
    const [item, setItem] = useAtom(itemAtom)
    const toggle = () => setItem((prev) => ({ ...prev, checked: !prev.checked }))
    return (
      <>
        <div>
  ...
  function List() {
    ...
    const addItem = () => {
      setItems((prev) => [...prev, atom<Item>({ text: `item${++itemIndex}`, checked: false })])
    }
    ...
    return (
      <ul>
        {items.map((itemAtom) => (
          <ListItem key={`${itemAtom}`} itemAtom={itemAtom} remove={() => removeItem(itemAtom)} />
        ))}
        <li>
...
it('add an item with filtered list', async () => {
  ...
  const setItemsAtom = atom(null, (_get, set, update: Update) => set(itemAtomsAtom, update))
  ...
  function FilteredList({ removeItem }: { removeItem: (itemAtom: PrimitiveAtom<Item>) => void }) {
