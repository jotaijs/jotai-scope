...
export function createScope({
  ...
  function getAtomScope(atom: AnyAtom): Scope | undefined {
    ...
    getAtom<T extends AnyAtom>(atom: T, implicitScope?: Scope): [T, Scope?] {
      ...
      if (!scopeMap.has(atom)) {
        ...
        if (atom.read === defaultRead) {
          ...
          const cloned = cloneAtom(atom, explicitScope)
          cloneToOriginal.set(cloned, [atom, explicitScope!])
          scopeMap.set(atom, [cloned, explicitScope])
        }
      }
  ...
  for (const atom of atomsSet) {
    const cloned = cloneAtom(atom, scope)
    cloneToOriginal.set(cloned, [atom, scope])
    explicit.set(atom, [cloned, scope])
  }
      ...
      return function scopedRead(get, opts) {
        return read(
          function scopedGet(a) {
            ...
            const [scopedAtom1] = scope.getAtom(a, implicitScope)
