...
it('should unmount with store.get', async () => {
  ...
  store.get(countAtom)
  unsub()
  const result = Array.from('dev4_get_mounted_atoms' in store ? store.dev4_get_mounted_atoms() : [])
...
it('should unmount dependencies with store.get', async () => {
  ...
  unsub()
  const result = Array.from('dev4_restore_atoms' in store ? store.dev4_get_mounted_atoms() : [])
  expect(result).toEqual([])
})
...
it('should not fire subscription when async atom promise is the same', async () => {
  ...
  const derivedGetter = jest.fn((get: Getter) => get(promiseAtom))
...
it('should notify subscription with tree dependencies (#1956)', async () => {
  ...
  const cb = jest.fn()
  const store = createStore()
  store.sub(dep2Atom, jest.fn()) // this will cause the bug
...
it('should bail out with the same value with chained dependency (#2014)', async () => {
  ...
  const deriveFn = jest.fn((get: Getter) => get(countAtom))
  const derivedAtom = atom(deriveFn)
  const deriveFurtherFn = jest.fn((get: Getter) => {
...
it('should not call read function for unmounted atoms (#2076)', async () => {
  ...
  const derive1Fn = jest.fn((get: Getter) => get(countAtom))
  const derived1Atom = atom(derive1Fn)
  const derive2Fn = jest.fn((get: Getter) => get(countAtom))
  ...
  store.sub(derived2Atom, jest.fn())
...
it('should update with conditional dependencies (#2084)', async () => {
  ...
  store.sub(f1, jest.fn())
  store.sub(f2, jest.fn())
  store.sub(f3, jest.fn())
  store.set(f3, true)
...
it('should not recompute a derived atom value if unchanged (#2168)', async () => {
  ...
  const derive2Fn = jest.fn((get: Getter) => get(derived1Atom))
  const derived2Atom = atom(derive2Fn)
...
it('should mount once with atom creator atom (#2314)', async () => {
  const countAtom = atom(1)
  countAtom.onMount = jest.fn((setAtom: (v: number) => void) => {
    setAtom(2)
  })
...
it('should flush pending write triggered asynchronously and indirectly (#2451)', async () => {
  ...
  const callbackFn = jest.fn((_value: string) => {})
...
describe('aborting atoms', () => {
  ...
  it('should abort the signal when dependencies change', async () => {
    const a = atom(1)
    const callBeforeAbort = jest.fn()
    const callAfterAbort = jest.fn()
    const resolve: (() => void)[] = []
