...
it('new atomFamily impl', async () => {
  ...
  function Displayer({ index }: { index: string }) {
...
it('read-only derived atomFamily returns same reference for same parameters', async () => {
  ...
  const myFamily = atomFamily((num: number) => atom((get) => get(arrayAtom)[num] as number))
  expect(myFamily(0)).toEqual(myFamily(0))
  expect(myFamily(0)).not.toEqual(myFamily(1))
  expect(myFamily(1)).not.toEqual(myFamily(0))
})
...
it('removed atom creates a new reference', async () => {
  ...
  const myFamily = atomFamily((num: number) => atom((get) => get(bigAtom)[num] as number))
...
it('primitive atomFamily initialized with props', async () => {
  ...
  function Displayer({ index }: { index: number }) {
  ...
  function Displayer({
    ...
    return (
      <div>
        ...
        <button onClick={() => setCount((oldValue) => oldValue + 1)}>increment #{index}</button>
      </div>
    )
  }
...
it('custom equality function work', async () => {
  ...
  const goodFamily = atomFamily(
    (num: { index: number }) => atom((get) => get(bigAtom)[num.index] as number),
    (l, r) => l.index === r.index,
  )
