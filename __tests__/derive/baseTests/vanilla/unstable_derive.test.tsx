...
describe('unstable_derive for scoping atoms', () => {
  ...
  it('primitive atom', async () => {
    ...
    const derivedStore = store.unstable_derive((baseGetAtomState) => {
      ...
      return [
        ...
        function atomReadTrap(atom, getter, options) {
          return atom.read(getter, options)
        },
        function atomWriteTrap(atom, getter, setter, ...args) {
  ...
  it('derived atom (scoping derived)', async () => {
    ...
    const derivedStore = store.unstable_derive((baseGetAtomState) => {
      ...
      return [
        function getAtomState(atom, originAtomState) {
          if (scopedAtomStateSet.has(originAtomState as never) || scopedAtoms.has(atom)) {
            let atomState = scopedAtomStateMap.get(atom)
            if (!atomState) {
              atomState = { d: new Map(), p: new Set(), n: 0 }
              scopedAtomStateMap.set(atom, atomState)
  ...
  it('derived atom (scoping derived chain)', async () => {
    ...
    function makeStores() {
      ...
      const deriStore = baseStore.unstable_derive((baseGetAtomState) => {
        const scopedAtomStateMap = new WeakMap()
        const scopedAtomStateSet = new WeakSet()
        return [
          function getAtomState(atom, originAtomState) {
    ...
    function getAtoms(store: Store) {
      return [store.get(a), store.get(b), store.get(c), store.get(d), store.get(e)]
    }
